...
class CreateFlight extends Component
{
    ...
    public function mount($countries, $cities, $categories)
    {
        ...
        $this->pricesItems[] = ['price_text_ar' => '', 'price_text_en' => '', 'price' => '', 'main_option' => true];
    ...
    public function submitForm()
    {
        ...
        if (!$flightCreated) {
            ...
            return redirect()->route('dashboard.flights.show', $flightCreated->id);
        }
    }
    ...
    public function isFormComplete()
    {
        ...
        if (!is_array($this->images) || count($this->images) === 0) return false;
    ...
    {
        // 1. Initial State: No interaction and no errors yet
        if (!$this->hasInteracted && !$this->getErrorBag()->any()) {
            return __('flights.hud_initial_guidance');
        }

        $tabsOrder = ['general', 'services', 'pricing', 'media', 'policies'];

        // 2. Error State: Check if current tab or any tab has errors
        ...
        // 3. Success State: Tab is valid, suggest next step
