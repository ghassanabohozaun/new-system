...
<div class="query-bar-container">
    <div class="query-bar js-query-bar">
        ...
        <form class="js-filter-form d-flex align-items-center gap-2">
            <!-- Title Filter Chip -->
            <div class="filter-chip js-filter-chip" data-target="title_popover">
                <i class="mdi mdi-pencil-box-outline"></i>
                <span class="chip-text">{!! __('tickets.title') !!}</span>
                <div class="filter-popover" id="title_popover">
                    <div class="mb-3 theme-primary">
                        <label class="form-label d-block fw-bold mb-1">{!! __('tickets.title_ar') !!}</label>
                    ...
                    <div class="mb-3 theme-primary">
                        <label class="form-label d-block fw-bold mb-1">{!! __('tickets.title_en') !!}</label>
                        <div class="input-group-premium">
                            <span class="input-group-text"><i class="mdi mdi-pencil-outline"></i></span>
            ...
            <div class="filter-chip js-filter-chip" data-target="price_popover">
                ...
                <span class="chip-text">{!! __('tickets.price') !!}</span>
                <div class="filter-popover" id="price_popover">
                    <div class="mb-3 theme-success">
                        <label class="form-label d-block fw-bold mb-1">{!! __('tickets.price') !!}</label>
                        <div class="input-group-premium">
            ...
            <!-- From Country/City Chip -->
            <div class="filter-chip js-filter-chip" data-target="from_popover">
                <i class="mdi mdi-map-marker-radius"></i>
                <span class="chip-text">{!! __('tickets.location') !!}</span>
                <div class="filter-popover" id="from_popover">
                    <div class="mb-3 theme-primary">
                        <label class="form-label d-block fw-bold mb-1">{!! __('tickets.from_country_id') !!}</label>
                        <div class="input-group-premium select2-premium">
                            <span class="input-group-text"><i class="mdi mdi-map-marker-radius"></i></span>
                            <select class="from_country_id_select form-control" name="from_country_id"
                                style="width: 100%">
                                @if (isset($fromCountry) && $fromCountry)
                                    <option value="{{ $fromCountry->id }}" selected
                                        {{ $fromCountry->flag_code ? 'data-flag-code=' . $fromCountry->flag_code : '' }}>
                                        {{ $fromCountry->getTranslation('name', app()->getLocale()) }}
                                    </option>
                                @endif
                                <option value=""></option>
                            </select>
                    ...
                    <div class="mb-3 theme-info">
                        <label class="form-label d-block fw-bold mb-1">{!! __('tickets.from_city_id') !!}</label>
                        <div class="input-group-premium select2-premium">
                            <span class="input-group-text"><i class="mdi mdi-city-variant-outline"></i></span>
                            <select class="from_city_id_select form-control" name="from_city_id" style="width: 100%">
                                @if (isset($fromCity) && $fromCity)
                                    <option value="{{ $fromCity->id }}" selected>
                                        {{ $fromCity->getTranslation('name', app()->getLocale()) }}
                                    </option>
                                @endif
                                <option value=""></option>
                            </select>
            ...
            <!-- To Country/City Chip -->
            <div class="filter-chip js-filter-chip" data-target="to_popover">
                <i class="mdi mdi-airplane-takeoff"></i>
                <span class="chip-text">{!! __('tickets.location') !!}</span>
                <div class="filter-popover" id="to_popover">
                    <div class="mb-3 theme-primary">
                        <label class="form-label d-block fw-bold mb-1">{!! __('tickets.to_country_id') !!}</label>
                        <div class="input-group-premium select2-premium">
                            <span class="input-group-text"><i class="mdi mdi-map-marker-radius"></i></span>
                            <select class="to_country_id_select form-control" name="to_country_id" style="width: 100%">
                                @if (isset($toCountry) && $toCountry)
                                    <option value="{{ $toCountry->id }}" selected
                                        {{ $toCountry->flag_code ? 'data-flag-code=' . $toCountry->flag_code : '' }}>
                                        {{ $toCountry->getTranslation('name', app()->getLocale()) }}
                                    </option>
                                @endif
                                <option value=""></option>
                            </select>
                    ...
                    <div class="mb-3 theme-info">
                        <label class="form-label d-block fw-bold mb-1">{!! __('tickets.to_city_id') !!}</label>
                        <div class="input-group-premium select2-premium">
                            <span class="input-group-text"><i class="mdi mdi-city-variant-outline"></i></span>
                            <select class="to_city_id_select form-control" name="to_city_id" style="width: 100%">
                                @if (isset($toCity) && $toCity)
                                    <option value="{{ $toCity->id }}" selected>
                                        {{ $toCity->getTranslation('name', app()->getLocale()) }}
                                    </option>
                                @endif
                                <option value=""></option>
                            </select>
            ...
            <div class="filter-chip js-filter-chip" data-target="status_popover">
                ...
                <span class="chip-text">{!! __('tickets.status_short') !!}</span>
                <div class="filter-popover" id="status_popover">
                    <div class="mb-3 theme-warning">
                        <label class="form-label d-block fw-bold mb-1">{!! __('tickets.status') !!}</label>
                        <div class="input-group-premium">
            ...
            $('.js-filter-chip').on('click', function(e) {
                ...
                if (!isVisible) {
                    $popover.fadeIn(200, function() {
                        ...
                        if (popId === 'from_popover' || popId === 'to_popover') {
                            const prefix = popId === 'from_popover' ? '.from_' : '.to_';
                            const $countrySelect = $popover.find(prefix + 'country_id_select');
                            const $citySelect = $popover.find(prefix + 'city_id_select');
